...
export class PartyManager {
    ...
    async disperseParty(partyToken, options = {}) {
        ...
        try {
            ...
            await this._restoreMembers(members, formattedPositions.map(() => partyPos));
            ...
            await this._animateDisperse(members, formattedPositions);
    ...
    async emergencyScatter(partyToken) {
        ...
        await this._restoreMembers(members, positions);
     ...
     * @param {Object[]} members - Member data array
     * @param {Array<{x: number, y: number}>} finalPositions - Target positions
     */
    async _animateDisperse(members, finalPositions) {
        // Small delay to let tokens render
        await new Promise(r => setTimeout(r, 100));

        // Find newly created tokens by actor ID
        const updateBatch = [];
        for (let i = 0; i < members.length; i++) {
            ...
            // Find the token on canvas by actor ID and name
            const token = canvas.tokens.placeables.find(t =>
                t.document.actorId === member.actorId && t.name === member.name
            );
            if (!token) continue;
